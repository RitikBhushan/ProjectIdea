...
const BlogDetails = (ctx) => {
    ...
    useEffect(() => {
      async function fetchComments(){
        const res = await fetch(`https://project-idea-xywm.vercel.app/api/comment/${ctx.params.id}`, {cache: 'no-store'})
    ...
    useEffect(() => {
        async function fetchBlog() {
            const res = await fetch(`https://project-idea-xywm.vercel.app/api/blog/${ctx.params.id}`, { cache: 'no-store' })
    ...
    const handleLike = async () => {
        try {
            const res = await fetch(`https://project-idea-xywm.vercel.app/api/blog/${ctx.params.id}/like`, {
                headers: {
                    'Authorization': `Bearer ${session?.user?.accessToken}`
                },
                method: 'PUT'
    ...
    const handleComment = async () => {
        ...
        try {
            ...
            const res = await fetch(`https://project-idea-xywm.vercel.app/api/comment`, {
                headers: {
    ...
    return (
        <div className={classes.container}>
            <div className={classes.wrapper}>
                <Image className={classes.detailImg} src={blogDetails?.imageUrl} width='750' height='650' />
                ...
                <div className={classes.row}>
                    ...
                    <div className={classes.category}>
                        Semester:
                        <span>{blogDetails?.sem}</span>
                    </div>
                    <div className={classes.right}>
                        {blogLikes} {" "} {isLiked ? <AiFillLike className={classes.like} size={20} onClick={handleLike} /> : <AiOutlineLike size={20} onClick={handleLike} />}
                    </div>
                </div>
                <div className={classes.row}>
